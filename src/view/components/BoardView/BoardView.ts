...
export class BoardView {
  ...
  onClickHandler = (onClick: (title: number) => void) => {
    ...
    setTimeout(() => {
      ...
      const parentProps = container?.getBoundingClientRect() as DOMRect;
      const emptyTileProps = document.querySelector('.empty-tile')?.getBoundingClientRect() as DOMRect;
      ...
      dragableTiles.forEach(tile => {
        ...
        ['mousedown', 'touchstart'].forEach(event => {
          ...
          tile.addEventListener(event, (e) => {
            const tile = <HTMLElement> e.target;
  ...
  render = () => {
    ...
    if(document.querySelector('.stat-moves-number')) {
      const statMovesNumber  = document.querySelector('.stat-moves-number') as HTMLElement;
      statMovesNumber.innerHTML = `${this.board.moves}`;
    }
  }
